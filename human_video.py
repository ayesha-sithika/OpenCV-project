...
def process_video_humans(model, video_path):
    """
    Process video and detect humans in each frame
    """
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    output_path = "out13.mp4"
    out = cv2.VideoWriter(output_path, fourcc, fps, (width, height))
    
    frame_count = 0
    total_humans = 0
    all_confidences = []  # ⭐ NEW: Store all confidence scores
    start_time = time.time()
    ...
    try:
        while True:
            ...
            # Process results
            human_count = 0
            frame_confidences = []  # ⭐ NEW: Confidence scores for current frame
            ...
            if results[0].boxes is not None:
                ...
                for box in results[0].boxes:
                    ...
                    frame_confidences.append(confidence)  # ⭐ NEW: Collect confidence
            ...
            # ⭐ NEW: Add frame confidences to overall list
            ...
            # ⭐ NEW: Show average confidence for current frame
            ...
            # Display the frame
            cv2.imshow('Video Human Detection - Press "q" to Stop', frame)
            cv2.setWindowProperty('Video Human Detection - Press "q" to Stop', cv2.WND_PROP_TOPMOST, 1)
            ...
            if frame_count % 50 == 0:
                ...
                # ⭐ NEW: Show current average confidence in progress
    ...
    print(f"   • Average confidence: {avg_confidence:.3f}")  # ⭐ NEW
    print(f"   • Confidence std dev: {confidence_std:.3f}")  # ⭐ NEW
    print(f"   • Output saved as: {output_path}")
    
    return output_path, processing_time, total_humans, frame_count, avg_confidence  # ⭐ UPDATED: Added avg_confidence
...
def live_camera_detection(model):
    ...
    try:
        while True:
            ...
            cv2.putText(frame, f"FPS: {fps:.1f}", (10, 60), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
            cv2.putText(frame, f"Avg Conf: {current_avg_confidence:.3f}", (10, 90),  # ⭐ NEW
            ...
            cv2.setWindowProperty('Real-Time Human Detection - Press "q" to Quit', cv2.WND_PROP_TOPMOST, 1)
    ...
    # ⭐ NEW: Calculate final average confidence
    avg_confidence = np.mean(all_confidences) if all_confidences else 0
    confidence_std = np.std(all_confidences) if all_confidences else 0
